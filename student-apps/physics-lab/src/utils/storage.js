...
const STORAGE_KEYS = {
  USER_PROFILE: '@physics_lab_user_profile',
  SETTINGS: '@physics_lab_settings',
  COMPLETED_EXPERIMENTS: '@physics_lab_completed',
  EXPERIMENT_RESULTS: '@physics_lab_results',
  FAVORITES: '@physics_lab_favorites',
  NOTES: '@physics_lab_notes',
  ACHIEVEMENTS: '@physics_lab_achievements',
};
...
export const exportLabData = async () => {
  try {
    ...
    const filename = `physics_lab_export_${Date.now()}.json`;
...
// Import shared data from other apps
export const importSharedData = async () => {
  try {
    const sharedDir = FileSystem.documentDirectory + '../shared_student_data/';
    const proficiencyPath = sharedDir + 'proficiency_data.json';

    const fileInfo = await FileSystem.getInfoAsync(proficiencyPath);
    if (fileInfo.exists) {
      const content = await FileSystem.readAsStringAsync(proficiencyPath);
...
// Export proficiency data to shared location
